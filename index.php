...
$db = new db('localhost', 'admin_xddialog', 'xddialog', 'admin_xddialog');
$add = false;
if (!isset($_COOKIE['userid']) || !(int)$_COOKIE['userid'] or !$db->exists('user', (int)$_COOKIE['userid'])) {
...
$xddialog = new xddialog($db, time(), $userid, $_REQUEST['hash'] ?? 'zPcfYXgp');
if ($add) {
...
// по идее следующие строки должны быть раскорячены, но так как у нас всего один диалог то они не нужны
...
// $xddialog->add_users_to_dialog($all_user);
    ...
    echo json_encode(get_messages(true), JSON_THROW_ON_ERROR);
} ?>
